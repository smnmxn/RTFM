...
# Phase 1: Claude detects the CSS framework and entry point
# Phase 1b: Compile CSS from source (npm install + sass/tailwind/postcss)
#           Falls back to CDN + theme overrides if compilation fails
# Phase 2: Claude writes the article + mockups using the compiled CSS

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "$SCRIPT_DIR/_shared.sh"

ARTICLE_TOPIC="${1:?Usage: b_with_css.sh \"Article topic e.g. How to reset your password\"}"

setup_output_dir "b"

echo "=== Approach B: Pre-extracted CSS → Article ==="
echo "Topic: $ARTICLE_TOPIC"
echo "Repo: $REPO_DIR"
echo ""

# ─── Phase 1: CSS Detection ──────────────────────────────────────────────────

echo "Phase 1: Detecting CSS framework and entry point..."
start_timer

cat <<'DETECT_PROMPT' | run_claude_streaming "$OUTPUT_DIR/css_detect_raw.json" --allowedTools "Read,Glob,Grep"
...
CSS_DETECT=$(jq -r '.result // empty' "$OUTPUT_DIR/css_detect_raw.json" 2>/dev/null | sed '/^```json$/d; /^```$/d' | sed -n '/^{/,/^}/p')

# Save parsed detection for debugging
echo "$CSS_DETECT" > "$OUTPUT_DIR/css_detect_parsed.json"

start_timer

# ─── Phase 1b: CSS Compilation ────────────────────────────────────────────────

CSS_FILE="$OUTPUT_DIR/compiled.css"
> "$CSS_FILE"
COMPILE_METHOD="none"
...
if [ -z "$CSS_DETECT" ]; then
    ...
    # ── Try 1: Compile from source (npm install + compiler) ──────────────
    ...
    _timeout() {
        if command -v gtimeout &>/dev/null; then
            ...
            shift  # drop the timeout value, just run the command
            "$@"
        fi
    }
    ...
    try_source_compile() {
        ...
        # Step 1: install deps (needed for all source compilation)
        ...
        # Step 2: Compile based on framework type
        case "$FRAMEWORK" in
            tailwind)
                if [ -n "$TW_ENTRY" ] && [ -f "$REPO_DIR/$TW_ENTRY" ]; then
                    ...
                    cat /tmp/compile_error.log | tail -5
                fi
                return 1
                ;;
    ...
    # ── Try 2: CDN + theme overrides (fallback) ─────────────────────────
...
PYEOF
    ...
    # ── Append additional CDN links and plain CSS files ──────────────────
...
# ─── Phase 1c: Image Extraction ──────────────────────────────────────────────

echo ""
echo "Phase 1c: Extracting images as base64 data URIs..."
start_timer

IMAGES_JSON="$OUTPUT_DIR/images_base64.json"

# Walk the repo for image files, skip node_modules/.git/vendor/tmp/build dirs
# No hardcoded asset paths — works with any framework
...
# Prioritise files likely to be brand/UI images
...
for item in found:
    try:
        ...
        # Skip if we already have this exact image (dedup by content)
        if data_uri in unique_data_uris:
            # Still map the path for lookup
        ...
        # Map under repo-relative path and filename
        images[rel] = data_uri
        images['/' + rel] = data_uri
        images[fname] = data_uri
        images['/' + fname] = data_uri
        # Map common web path patterns: /assets/X, /images/X, /img/X
...
# Create manifest for Claude to read (just filenames, no data)
IMAGE_MANIFEST="$OUTPUT_DIR/images_manifest.txt"
jq -r '.images | keys[]' "$IMAGES_JSON" 2>/dev/null | sort -u > "$IMAGE_MANIFEST"

record_phase "image_extraction"

# ─── Phase 2: Article Generation with CSS ────────────────────────────────────

echo ""
echo "Phase 2: Generating article with pre-compiled CSS..."
start_timer
...
Write each mockup to: ${OUTPUT_DIR}/html/step_N.html (N is 0-based)
...
python3 - "$OUTPUT_DIR/html" "$CSS_FILE" "$IMAGES_JSON" <<'PYEOF'
...
echo "CSS:     $CSS_FILE ($COMPILE_METHOD, $CSS_SIZE bytes)"
