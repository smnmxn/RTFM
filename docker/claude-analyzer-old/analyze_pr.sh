...
# - VCS_REPOS_JSON or GITHUB_REPOS_JSON: JSON array of {repo, directory, token, clone_url} objects
...
# - context.json: Project context and PR metadata (includes repository_relationships for multi-repo)
...
# Support VCS_REPOS_JSON with GITHUB_REPOS_JSON fallback
REPOS_JSON="${VCS_REPOS_JSON:-${GITHUB_REPOS_JSON}}"
...
if [ -n "${REPOS_JSON}" ]; then
    ...
    for i in $(seq 0 $((REPO_COUNT - 1))); do
        ...
        DIR=$(echo "$REPOS_JSON" | jq -r ".[$i].directory")
        TOKEN=$(echo "$REPOS_JSON" | jq -r ".[$i].token")
        CLONE_URL=$(echo "$REPOS_JSON" | jq -r ".[$i].clone_url // empty")

        # Use clone_url if provided, fall back to GitHub URL
        if [ -z "$CLONE_URL" ]; then
...
# Read PR number from context for logging
PR_NUMBER=$(cat /input/context.json | grep -o '"pr_number":[0-9]*' | grep -o '[0-9]*' || echo "unknown")
...
This PR affects the repository: $(cat /input/context.json | jq -r '.source_repo // .github_repo // "unknown"')

When analyzing this PR, consider how changes might affect other repositories in this project.
"
    else
...
# Run Claude Code to analyze the PR diff
...
STEP 2: Read the PR diff to understand what code changed:
/input/diff.patch
...
{"articles":[{"title":"How to [do the thing]","description":"One sentence describing what this guide would cover","justification":"Why this guide is needed based on the PR changes"}],"no_articles_reason":null}
