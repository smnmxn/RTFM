...
# - CLONE_URL: Full clone URL (preferred, provider-agnostic)
#   OR:
...
# Clone the repository using CLONE_URL if available, fall back to GitHub URL
REPO_CLONE_URL="${CLONE_URL:-https://x-access-token:${GITHUB_TOKEN}@github.com/${GITHUB_REPO}.git}"
echo "Cloning repository..."
if ! git clone "$REPO_CLONE_URL" /repo 2>&1; then
